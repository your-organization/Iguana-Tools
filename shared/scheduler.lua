...
-- a table named "arg" - these are passed to the function
-- by converting them using the "unpack" function
local function runAt2(T)
   -- T.start, T.end -- both date + time
   -- T.period -- H, D, W, M, minutes, one-off
   -- T.func, T.args
   
--local function runAt2(scheduledStart, scheduledEnd, func, period, ...)
   local R
   local LastRunTime = LastRunTimes[tostring(T.func)..' + Time: '..tostring(T.Start)]
   trace(LastRunTime) 
   if LastRunTime == 0 or LastRunTime == nil or iguana.isTest() then
      -- We need to do one time initialization
      R, LastRunTime = Init(scheduledHour, func)
